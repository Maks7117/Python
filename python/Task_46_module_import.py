...
#
# – для пакета 'triangle':
# функции triangle_perimeter() – вычисляет периметр треугольника,
# triangle_area() – вычисляет площадь фигуры.
# Дополнительно создадим три переменные
# (длины сторон треугольника): a = 7, b = 2, c = 8,
# которые также не будут видны при импорте.
# На вход функциям передается длина трех сторон
# (если пользователь ничего не введет, то используются значения по умолчанию).
#
# – для пакета 'square':
# функции square_perimeter() – вычисляет периметр квадрата,
# square_area() – вычисляет площадь фигуры.
# Дополнительная переменная a = 15 не доступна при импорте
# и принимается функциями, если пользователь не предоставил свои размеры стороны квадрата.
#
# Ваша итоговая задача – позволить человеку,
# загрузившему ваш пакет, иметь возможность
# напрямую импортировать все функции из подпакетов.
# Например, он может написать так: 'from figure import circle_area'.
#
# Также вы, как разработчик, после написания всей
# библиотеки решили поменять ее имя на 'figures'.
# Постарайтесь сделать код таким, чтобы это не заставило
# вас переписывать все внутренние импорты с учетом нового именования.